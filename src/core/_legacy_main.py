...
An open-world 2D gacha RPG built with pygame (170 LoL champions).
Run:  python3 main.py
"""
import math
import random
...
import audio
# Shared UI primitives (fonts, text cache, Button, draw_bar/panel/stars, dim
# overlay, color lookups). Extracted to ui.py so the world/adventure scenes can
# import them without forcing a main <-> world_scene circular import.
from ui import (WIDTH, HEIGHT, FPS, TITLE, SEED,
                WHITE, DIM, GOLD, PANEL, PANEL_BORDER,
                HP_RED, HP_GREEN, MP_BLUE, XP_PURPLE, BG_DARK,
                FONTS, init_fonts, get_font, f, text,
                Button, draw_panel, draw_bar, draw_stars,
                dim_overlay as _dim_overlay, element_color, rarity_color,
                scratch as _scratch,
                Toggle, Slider)
# Back-compat re-exports: world_scene imports Button + draw_bar from main, and
# other modules may reference main.text / main.get_font. Keep these names live
# on the main module until all callers import from ui directly.
random.seed(SEED)

# Open-world scene: imported lazily (it transitively imports generate_assets +
# world_data, which are heavy). Resolved the first time the world scene is
# created. The old circular-import hazard (world_scene imported Button/draw_bar
# from main) is gone — those now come from ui.py.
...
# Adventure scene: imported lazily for the same reason (it subclasses
# WorldScene). Resolved the first time the adventure scene is created.
...
# Toggle + Slider widgets for the settings menu now live in src/ui/widgets.py
# (imported above via `from ui import Toggle, Slider`). The colorblind-aware
# element_color reads Game._active via a late import so ui doesn't depend on
# main.
