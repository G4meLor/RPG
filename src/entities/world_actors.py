...
from src.data.tuning import TOUGHNESS_BREAK_DAMAGE, TOUGHNESS_BREAK_MULT
...
class WorldCharacter:
    ...
    def update(self, dt, input_dir, obstacles, want_dash):
        # Task 20d: the movement body is ported verbatim into
        # PhysicsSystem.update_hero (src/systems/physics.py). This delegate
        # keeps the wc as the operated-on subject (the body operates on the
        # wc's own fields: x, vx, atk_cd, hero, etc.). The swap to entity
        # components is Task 20e.
        from src.systems.physics import PhysicsSystem
        PhysicsSystem.update_hero(self, dt, input_dir, obstacles, want_dash)
...
class WorldEnemy:
    ...
    def update(self, dt, target, obstacles, projectiles, particles, on_attack):
        # Task 20d: the AI body is ported verbatim into AISystem.update_enemy
        # (src/systems/ai.py). This delegate keeps the en as the operated-on
        # subject (the body operates on the en's own fields: x, vx, state,
        # enemy, etc.) + the params (target, obstacles, projectiles, particles,
        # on_attack). Helper calls (en._do_attack / en._collide /
        # en.take_damage) are WorldEnemy methods, unchanged. The swap to entity
        # components is Task 20e.
        from src.systems.ai import AISystem
        AISystem.update_enemy(self, dt, target, obstacles, projectiles, particles, on_attack)
