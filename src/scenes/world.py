...
class WorldScene:
    ...
    def draw(self, surf):
        ...
        # (_draw_hud, Task 20b) is still drawn via self.scene._draw_hud inside
        # RenderSystem.draw until HudSystem ports it.
        return self.render.draw(surf, self.map_ctrl)

    # Task 20a: _draw_chevron was ported into RenderSystem (the edge-hint + the
    # quest compass both draw chevrons). The HUD's quest compass (_draw_hud,
    # still on WorldScene until Task 20b) reuses it via this thin delegate so
    # the verbatim call site `self._draw_chevron(...)` keeps working.
    def _draw_chevron(self, surf, x, y, d, col, pulse):
        return self.render._draw_chevron(surf, x, y, d, col, pulse)
