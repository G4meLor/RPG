...
overlay, and the element/rarity color lookups. Extracted from main.py so the
world/adventure scenes can import them without pulling in the whole game
loop (which previously forced a main <-> world_scene circular import).

This module depends only on pygame + data + audio + world_entities.scratch,
so it is safe to import from any scene at module load time.
"""
import math
